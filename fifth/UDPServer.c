#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <netdb.h>
#include <sys/types.h>
#include <sys/socket.h>
#include <arpa/inet.h>

#define BUFSIZE 32

void DieWithError(char *msg);

int main(int argc, char * argv[]){

    if(argc !=2){
    
        fprintf(stderr,"Usage: %s <Server Port>\n",argv[0]);
        exit(1);
    }
    
    char * port= argv[1];
    struct addrinfo addrCriteria;
    struct addrinfo *serverAddr;

    memset(&addrCriteria,0, sizeof(addrCriteria));
    addrCriteria.ai_family=AF_UNSPEC;
    addrCriteria.ai_flags=AI_PASSIVE;
    addrCriteria.ai_socktype=SOCK_DGRAM;
    addrCriteria.ai_protocol=IPPROTO_UDP;

    int returnValue= getaddrinfo(NULL,port, &addrCriteria, &serverAddr);
    
    if(returnValue !=0)
        DieWithError("getaddrinfo() failed");
    int sock=socket(serverAddr->ai_family, serverAddr->ai_socktype, serverAddr->ai_protocol);
   // if((sock=socket(serverAddr->ai_family, serverAddr->ai_socktype, serverAddr->ai_protocol))<0)
     //   DieWithError("socket() failed");
    if(sock<0)
        DieWithError("socket() failed");

    if(bind(sock, serverAddr->ai_addr, serverAddr->ai_addrlen)<0)
        DieWithError("bind() failed");
    printf("bind success!\n");
    freeaddrinfo(serverAddr);

    char buffer[BUFSIZE];
    
    memset(buffer,'\0',BUFSIZE);
    
    while(strcmp(buffer, "./quit"))
    {
    
        struct sockaddr_storage clientAddr;
        socklen_t clientAddrLen= sizeof(clientAddr);
        ssize_t numByteRcvd = recvfrom(sock, buffer, BUFSIZE,0,(struct sockaddr*)&clientAddr, &clientAddrLen);
        if(numByteRcvd<0)
            DieWithError("recvdfrom() failed");

        buffer[numByteRcvd]='\0';
        printf("msg<- %s\n", buffer);

        ssize_t numByteSend =sendto(sock, buffer, numByteRcvd, 0, (struct sockaddr *)&clientAddr,sizeof(clientAddr) );
        if(numByteSend <0)
            DieWithError("sendto() failed");
        else if(numByteSend != numByteRcvd)
            DieWithError("sendto sent unexpected number of bytes");

        buffer[numByteSend]='\0';
        printf("msg-> %s\n",buffer);
    }
    close(sock);
    return 0;
}
